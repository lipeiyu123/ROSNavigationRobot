...
class following_path:
 ...
 #       self.current_pose = rospy.Subscriber('/pf/pose/odom', Odometry, self.callback_read_current_position, queue_size=1)
        ...
        #self.current_pose = rospy.Subscriber('/rf2o_laser_odometry/odom', Odometry, self.callback_read_current_position, queue_size=1)
        ...
        self.MAX_VELOCITY = 1.0 #0.5  0.3  1.0
        self.MIN_VELOCITY = 0
        self.max_angle = 1
        self.steering_velocity = 1.0  #1  0.5
        self.jerk = 0.0
    ...
    def callback_read_current_position(self, data):
        ...
        if not len(self.path_info) == 0:
            ...
            if self.dist(self.Goal, self.Pose) < 1.0:    #1.0
                self.Low_Speed_Mode = True  #True
                if self.dist(self.Goal, self.Pose) < 0.3:
                    self.reach_goal = False  #True
                    print('Goal Reached!')
                else:
