...
L1Controller::L1Controller()
{
    ...
    odom_sub = n_.subscribe("/odometry/filtered", 1, &L1Controller::odomCB, this);//订阅的里程计信息
    path_sub = n_.subscribe("/move_base_node/NavfnROS/plan", 1, &L1Controller::pathCB, this);//订阅的规划的路径信息
    goal_sub = n_.subscribe("/move_base_simple/goal", 1, &L1Controller::goalCB, this);//订阅的目标点信息
    marker_pub = n_.advertise<visualization_msgs::Marker>("car_path", 10);//可视化           //发布给rviz的小车路径信息
    pub_ = n_.advertise<geometry_msgs::Twist>("car/cmd_vel", 1);//几何学                    //发布的额控制信息


    //Timer  定时调用控制函数   与  检测目标点 函数
    ...
    //Init variables变量 
    Lfw = goalRadius = getL1Distance(Vcmd);//拟定一个最小距离
    foundForwardPt = false;   //找到前进点
    goal_received = false;    //收到
    goal_reached = false;     //达到
    ...
    ROS_INFO("[param] AngleGain: %f", Angle_gain);//获得角度
    ROS_INFO("[param] Vcmd: %f", Vcmd);
    ROS_INFO("[param] Lfw: %f", Lfw);

    //Visualization Marker Settings 可视化标记设置
    initMarker();   
    car_stop = 0;
}
...
void L1Controller::controlLoopCB(const ros::TimerEvent&)              //pid
{
    int count = 100;
